...
@WebServlet("/AddressChangeDashBoard")
public class AddressChangeDashBoard extends HttpServlet {
    String vPass, vUsername, vaccNum, vaccType, vEMAIL, vNAME, vADDRESS, vCITY, vPIN, vSTATE, vDISTRICT;
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            vPIN = request.getParameter("pin");
            vDISTRICT = request.getParameter("district");
            ...
            try {
                ...
                ops = (OraclePreparedStatement) oconn.prepareCall(
                        "UPDATE CUSTOMER SET ADDRESS = (?), CITY = (?), PIN = (?), DISTRICT = (?), STATE = (?) WHERE EMAIL = (?)");

                // STEP 6: FILLING UP THE BLANK QUERY PARAMETERS (?)

                ops.setString(1, vADDRESS);
                ops.setString(2, vCITY);
                ops.setString(3, vPIN);
                ops.setString(4, vDISTRICT);
                ops.setString(5, vSTATE);
                ops.setString(6, vEMAIL);
            ...
            // STEP 9: FORMATTING THE CATCH CLAUSE
                ...
                Logger.getLogger(AddressChangeDashBoard.class.getName()).log(Level.SEVERE, null, ex);
